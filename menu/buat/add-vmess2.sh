...
cat>/etc/v2ray/vmess-$user-ws.json<<EOF
      {
      "v": "2",
      "ps": "${user}",
      "add": "${domain}",
...
EOF
cat>/etc/v2ray/vmess-$user-wstls.json<<EOF
      {
      "v": "2",
      "ps": "${user}",
      "add": "${domain}",
...
EOF
cat>/etc/v2ray/vmess-$user-grpc.json<<EOF
      {
      "v": "2",
      "ps": "${user}",
      "add": "${domain}",
...
vmesslinkws="vmess://$(base64 -w 0 /etc/v2ray/vmess-$user-ws.json)"
vmesslinkwstls="vmess://$(base64 -w 0 /etc/v2ray/vmess-$user-wstls.json)"
vmesslinkgrpc="vmess://$(base64 -w 0 /etc/v2ray/vmess-$user-grpc.json)"
...
echo -e "Path        : /v2rayvws/vmess-grpc"
...
echo -e
echo -e "========================="
echo -e "AKCELL v2ray MULTI VMESS"
